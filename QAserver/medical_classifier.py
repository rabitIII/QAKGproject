...
class QuestionClassifier:
    def __init__(self):
        ...
        # 构造领域actree，基于树匹配比关键词分割匹配更高效，ahocorasick是一个现成的快速匹配函数
    ...
    def classify(self, question):
        ...
        # 症状
        # 查找self.symptom_qwds来自于init，查找self.symptom_qwds是否在question内
        if self.check_words(self.symptom_qwds, question) and ('disease' in types):
            question_type = 'disease_symptom'
            question_types.append(question_type)
        # check_words是下面定义的特征词分类函数
    ...
    def build_wdtype_dict(self):
        wd_dict = dict()
        for wd in self.region_words:    # 检索用户输入的词是什么范围的，比如用户输入高血压，这个单词属于疾病？食物？科室？还是药物？
            wd_dict[wd] = []
            ...
            if wd in self.check_wds:
                wd_dict[wd].append('check')         # 检查
            if wd in self.drug_wds:
                wd_dict[wd].append('drug')
            if wd in self.food_wds:
                wd_dict[wd].append('food')          # 饮食归类
            if wd in self.symptom_wds:
                wd_dict[wd].append('symptom')       # 症状是否归属于疾病
